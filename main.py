...
# ─────────────────────────────────────────────
BOT_TOKEN = os.getenv("BOT_TOKEN", "8822467436:AAGnm5fI0QWa787Wr0yXj4ho3yOt2f5UIFE")
GITHUB_TOKEN = os.getenv("GITHUB_TOKEN", "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")
ADMIN_ID = os.getenv("ADMIN_ID", "7262926892")
REPO_OWNER = os.getenv("REPO_OWNER", "Almax95")
REPO_NAME = os.getenv("REPO_NAME", "pp")

# Semaphore limits concurrent requests to prevent server exhaustion
...
async def update_file_content(path, content, sha, message):
    ...
    try:
        ...
        print(f"Error updating {path} on GitHub: {e}")
        return None

# ─────────────────────────────────────────────
#  Voucher Code Generators & Iterators
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
#  Voucher Balance / Plan Fetcher
# ─────────────────────────────────────────────
def Minute_to_Hour(total_minutes):
    if total_minutes == 'Unknown' or total_minutes is None:
        return 'Unknown'
    try:
...
async def Code_Expires_Date(active_session_id):
    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=_connector, connector_owner=False, timeout=timeout) as fresh_session:
            url = f'https://portal-as.ruijienetworks.com/api/macc2/balance/getBalance/{active_session_id}'
            async with fresh_session.get(url, headers=headers) as req:
                respond = await req.json()
                result_data = respond.get('result', {})
                profile_name = result_data.get('profileName', 'Unknown')
                totaltime = Minute_to_Hour(result_data.get('totalMinutes', 'Unknown'))
...
async def perform_check(session_url, code, chat_id, scan_id=None, recheck=False, message=None):
        ...
        async with aiohttp.ClientSession(connector=_connector, connector_owner=False, timeout=timeout) as task_session:
            ...
            for _ in range(8):
                try:
                    ...
                    print(f"Captcha extraction warning: {e}")
    ...
    # Extract Active Token to retrieve plan balance
    active_session_id = session_id
    try:
        match = re.search(r'token=([^&"\'\s>]+)', response)
        if match:
            active_session_id = match.group(1)
    except Exception:
        pass
    ...
    if 'logonUrl' in response:
        ...
        if message:
            try:
                ...
                print(f"Failed to deliver Telegram success output: {e}")
        ...
        if message:
            try:
                ...
                print(f"Failed to deliver Telegram limited output: {e}")

# ─────────────────────────────────────────────
#  Access Key System Helpers
# ─────────────────────────────────────────────
    ...
    while True:
        ...
        if items:
            try:
                ...
                print(f"Scheduled GitHub storage update failed: {e}")
...
async def handle_input(message):
    ...
    if not approve.get(chat_id, False):
        await bot.reply_to(message, "ဦးစွာ /start ကိုပြုလုပ်ပေးပါ။")
        return
...
async def recheck(message):
    chat_id = message.chat.id
    if not approve.get(chat_id, False):
        await bot.reply_to(message, "/recheck ကိုအသုံးမပြုမီ /start ကိုအရင်ပြုလုပ်ပေးပါ။")
        return
    results, sha = await get_file_content("result.json")
...
# ─────────────────────────────────────────────
#  Infinity Polling / Reconnect Loop
# ─────────────────────────────────────────────
